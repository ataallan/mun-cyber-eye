...
4. Open an alert → `camera_id` + `location_label` from the registry, play the short incident clip when monitoring saved one, then acknowledge / dismiss / escalate / reopen (alerts are retained — no delete wipe). **Archive** is optional and off (`ARCHIVE_ENABLED=0`); turn it on to keep continuous segments (1 day by default, or 5) separate from those clips.  
