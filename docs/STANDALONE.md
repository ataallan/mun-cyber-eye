...
- **Archive** is optional and off by default (`ARCHIVE_ENABLED=0`). When you turn it on, enabled cameras record continuously while monitoring is on, in short segments under `data/archive/`. Retention defaults to **1 day** (1 and 5 days are in the console). Old segments are removed by age. That is not a wipe of alert rows. Incident clips stay in `data/clips/`. At 8 fps, plan on about **0.3–1.1 GB per camera per hour** (about **7–26 GB per camera per day**; five days is about five times that). A full disk or a dead stream is reported; footage is not invented.
