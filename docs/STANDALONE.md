...
- **Video file uploads are for training only** (admin Train models → Extract frames from video). Run Pipeline is not a customer “upload an incident clip for live detection” path.
- **No default credentials.** Create the first admin on **Create account** with a password you choose.
...
5. Create the first admin account (this becomes site admin). Later Create account users are operators.
...
4. **Train models** (admin) — extract frames from authorized video into a labeled dataset; then train.
