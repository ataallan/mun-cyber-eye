...
4. Review dataset counts under `data/activity/{train,val,test}/<category>/` (the six canonical classes, including `game_or_play` / `dance` / `potential_fight`) and any sport-context folder totals.
...
9. **Upload labeled frames** — multi-file into a chosen category (optional sport context writes `game_or_play__<sport>`), or a zip of `train/<category>/*.jpg` including catalog sport folders. Unknown category names and `..` paths are rejected. Sport-context and body-aggression assists stay in the pipeline; training does not replace them.
...
- Upload authorized, site-specific examples for the six classes, including game/dance and optional sport folders. Synthetic-only sport courts do **not** mean the model knows most sports.
