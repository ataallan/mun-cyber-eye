...
The Flask console, SQLite alerts, and reviewer actions (acknowledge / dismiss / escalate) stay on the same contract. Mun Cyber developers can also train and activate a checkpoint from **Train models** (`/admin/train`) — see [ADMIN_TRAINING.md](ADMIN_TRAINING.md). Customers use the shipped checkpoint.
...
1. **Train models** (`/admin/train`, developer only) — dataset counts, train, evaluate, activate, upload labeled frames (including sport folders). Customer site admins do not see this page.
...
4. Point `ACTIVITY_CHECKPOINT` at the new `.joblib`, or activate it from **Train models** on a developer lab account (writes `data/active_checkpoint.json`; no secrets file is edited).
