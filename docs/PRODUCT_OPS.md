...
After approval, customer `admin` / `operator` accounts must enter a **one-time code emailed to their login address** (`users.email`) before the console opens. `security_email` remains the preferred alert-notify address and is not used for sign-in codes. If Resend is not configured, the UI says so honestly; set `AUTH_SHOW_LOGIN_CODE=1` for local demos, or `CUSTOMER_2FA_REQUIRED=0` to skip the second gate. Mun Cyber `developer` lab accounts skip email 2FA unless `DEVELOPER_2FA_REQUIRED=1`.
