...
Fresh installs ship **no default password**. Create the first **site admin** with **Create account** — that first account is **auto-approved** so the product is not locked out. Passwords must be **at least 12 characters** with **at least one letter and one digit**; `changeme`, `password`, `admin`, `operator`, and obvious sequences are rejected. Later public registrations are operators and stay **pending** until a site `admin` or `developer` approves them on **Accounts**. Unapproved users cannot sign in to the console, cameras, Run Pipeline, or alerts; password reset does not bypass that gate.

**2FA is not enforced on the Mun Cyber Eye Capstone console.** Access is password plus admin approval only (separate from any MFA on www.muncyber.com).

Site admin and operator run cameras, alerts, My cameras, recipients, and Run Pipeline **after approval** — they **cannot** train. Optional `ADMIN_USERNAME` / `ADMIN_PASSWORD` bootstrap only if you set both yourself — never a published default. Env-seeded `ADMIN_*`, `OPERATOR_*`, and `DEVELOPER_*` accounts are active/approved when created, and those passwords must pass the same policy. `operator` / `changeme` cannot be seeded. On boot (`DISABLE_LEGACY_OPERATOR=1`, default) a leftover Capstone `operator` row is deactivated only if the password is still `changeme` or the email is `operator@localhost`. Run `python scripts/disable_legacy_operator.py` for a one-shot cleanup. Sign-in as `operator` / `changeme` is always rejected.
