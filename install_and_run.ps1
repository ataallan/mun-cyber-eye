# Mun Cyber Eye - Windows standalone installer
# First run: create .venv, install requirements.txt, write install.log,
# create the Desktop and folder shortcuts, then start the console.
# Later runs: when .venv is healthy, offer Start only (default) or Upgrade.
# Ultralytics and torch are optional and are not installed.
...
$script:Launcher = Join-Path $PSScriptRoot "Start Mun Cyber Eye.bat"
$script:Icon = Join-Path $PSScriptRoot "app\static\img\mun-cyber-eye.ico"
...
function Show-Failure {
    ...
    if ($risky) {
        ...
        Write-Host "Copy the unzipped folder to C:\MunCyberEye and run install_and_run.bat from there."
    }
    Write-Host ""
...
function Install-Requirements {
    ...
    Write-Host "Ultralytics and torch stay optional and are not installed."
...
function Repair-OptionalLeftovers {
    ...
    if ($removed) {
        Write-Host "Removed optional ultralytics/torch leftovers: $removed"
        Write-Host "Core requirements stay installed."
        Write-InstallLog "scrubbed optional leftovers: $removed"
    }
}

function Install-MunShortcuts {
    ...
    $shortcutPaths = @(
        (Join-Path $desktop "Mun Cyber Eye.lnk"),
        (Join-Path $PSScriptRoot "Mun Cyber Eye.lnk")
    )
    foreach ($shortcutPath in $shortcutPaths) {
        $shortcut = $shell.CreateShortcut($shortcutPath)
        $shortcut.TargetPath = $script:Launcher
        $shortcut.WorkingDirectory = $PSScriptRoot
        $shortcut.Description = "Mun Cyber Eye"
    ...
    Write-Host "Next time, open Mun Cyber Eye from the Desktop shortcut."
}
...
function Start-MunEye {
    Copy-EnvExample
    try {
        Install-MunShortcuts
    } catch {
        Write-Host "Could not create shortcuts: $($_.Exception.Message)"
        Write-Host "The console will still start. You can run Start Mun Cyber Eye.bat in this folder."
    }
    ...
    Write-Host ""
    & $script:VenvPython (Join-Path $PSScriptRoot "run.py")
    $code = $LASTEXITCODE
    Write-Host ""
    Pause-Close "Press Enter to close"
    exit $code
...
if ($action -eq "ready") {
    Write-Host "Mun Cyber Eye is already installed in .venv."
    ...
    if ($choice -match '^[Uu]') {
        ...
        $after = Get-VenvAssessment -Python $python
        if ([string]$after.action -eq "scrub_optional") {
            Repair-OptionalLeftovers -Python $python
        } elseif ([string]$after.action -ne "ready") {
            Show-Failure -Message "Upgrade finished, but core packages still failed to import. $($after.detail)" -Python $python
            exit 1
        }
    }
    Start-MunEye
...
if ($reason -eq "damaged") {
    ...
    Write-Host "Optional ultralytics/torch leftovers are cleared with it. They are not required."
...
if ($afterAction -ne "ready") {
    Show-Failure -Message "Packages were installed, but core imports still failed (flask, cv2, and the rest of requirements.txt). $($afterInstall.detail)" -Python $python
    exit 1
}
